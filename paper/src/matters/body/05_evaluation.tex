...
\noindent
Intermediate tiers reveal power grid failure (Level~B1) as Trier's primary cascading bottleneck: Level~B1 matches Level~C identically ($RoA_{\text{Int}} = 78.20\,\%$, nadir $37.29\,\%$), whereas Level~B2 maintains higher resilience ($RoA_{\text{Int}} = 78.84\,\%$, nadir $38.41\,\%$).
This indicates that water networks are less vulnerable to flooding than power infrastructure.
This is attributed to the location of water towers, which are situated farther away from the Mosel than power infrastructure and are therefore less likely to be affected by flooding.
...
This is confirmed: emergency power buffers delay facility collapses by up to $24\,\text{hours}$ during flood rise.
This 48-hour recovery lag validates the operational penalty of FSM reboot latencies ($T_{\text{repair}} \in \{24\,\text{h for fire.}, 48\,\text{h for hosp.}\}$),
While the buffer delay can be observed in Figure~\ref{fig:multi-tier-ts} while the recovery lag can be observed in Figure~\ref{fig:multi-tier-ts}.

\subsection{Spatial Vulnerability: Municipal Patterns and Service Disparity}
\label{subsec:spatial-vulnerability}

Figure~\ref{fig:spatial_resilience_map} maps accessibility loss ($\Delta RoA_j$) across Trier at HQ100, comparing the transport baseline (Level~A) to compound failure (Level~C), where river inundation severs primary transit arteries.
Because healthcare facilities ($N=3$) cluster along the river axis, road closures eliminate access entirely, reducing hospital accessibility to $0.00\%$ across all tiers, access is thus strictly transport-dominated.
Conversely, fire stations ($N=12$) distributed across municipal districts provide redundancy, preserving a $76.82\%$ nadir in Level~A. Under Level~C, utility cascades reduce this to $74.59\%$ (Figure~\ref{fig:spatial-gap}), though Ruwer-Eitelsbach remains operational throughout the buffer despite power-station flooding.
