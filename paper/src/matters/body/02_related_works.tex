...
Existing methods for assessing CI resilience consider various behavioral aspects during disruptions~\cite{calvert2017,kaub2024,lickert2024}.
...
CIs are complex, interconnected networks~\cite{chiaradonna2007}.
...
Following these lifetimes and \textit{repair times}, systems transition between \textit{operative} and \textit{dysfunctional} states~\cite{lickert2024}.
...
Disruptions reduce this serviceability, increasing travel times~\cite{kaub2024}.
...
\begin{equation}
    \text{RoA}(\mathcal{C}) = \sum_{j=1}^{m} \left( a_{\mathcal{C},j} \cdot \frac{c(s_j,d_j)}{c'(s'_j, d'_j)} \right) \label{eq:old_roa}
\end{equation}

\noindent
RoA values range from 0 (complete failure) to 1 (full resilience)~\cite{kaub2024}.
If a destination becomes completely unreachable, the ratio is set to 0~\cite{kaub2024}.
While Kaub et al.\ focus on road accessibility, they omit functional facility states~\cite{kaub2024}.
