...
\begin{enumerate}
  \item \textbf{Road Network ($G$)}: Filtered to drivable roads, yielding an undirected graph of $|V| = 5{,}817$ nodes with traversal times $c(e) = \text{length}(e)/\text{speed}(e)$.
  \item \textbf{Critical POIs and Utilities}: Destinations comprise $3$ acute hospitals and $12$ fire stations.
...
The multi-tier runner evaluates four resilience tiers under uniform recovery parameters ($\theta = 0.15$ hysteresis threshold, reboot delay $T_{\text{repair}} = 24\,\text{h}$ for fire stations and $48\,\text{h}$ for hospitals):

\begin{itemize}
  \item \textbf{Level~A}: Infinite power and water buffers ($B_{\text{power}} = B_{\text{water}} = \infty$) across all facilities, isolating pure road network disruptions as a baseline.
  \item \textbf{Level~B1}: Infinite water buffers ($B_{\text{water}} = \infty$) with finite power reserves ($B_{\text{power}} = 24\,\text{h}$ for fire stations, $72\,\text{h}$ for hospitals).
  \item \textbf{Level~B2}: Infinite power buffers ($B_{\text{power}} = \infty$) with finite water reserves ($B_{\text{water}} = 12\,\text{h}$ for fire stations, $48\,\text{h}$ for hospitals).
  \item \textbf{Level~C}: Compound power and water buffers combining the finite reserve constraints of Levels~B1 and~B2.
\end{itemize}
